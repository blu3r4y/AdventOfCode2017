...
def part2(graph):
    ...
    def calculate_weights(node):
        def _weights(node, return_tuples=True):
            if return_tuples:
                return [(child[0], sum(_weights(child[0], return_tuples=False)) + child[1]) for child in children(node)]
            return [sum(_weights(child[0], return_tuples=False)) + child[1] for child in children(node)]
